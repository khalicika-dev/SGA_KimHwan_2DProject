...
DirectWrite::DirectWrite()
	: device(nullptr)
{
	//V(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), (IUnknown**)&writeFactory));
	...
	//V(device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS, &context));
	device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS, &context);

	IDXGISurface* dxgiSurface;
	//V(Device::Get()->GetSwapChain()->GetBuffer(0, __uuidof(IDXGISurface), (void**)&dxgiSurface));
...
void DirectWrite::RenderText(wstring text, RECT rect, float size, DWRITE_TEXT_ALIGNMENT align, wstring font, Float4 color, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
	...
	if (findText == nullptr)
	{
		...
		textDesc.format->SetTextAlignment(align);
