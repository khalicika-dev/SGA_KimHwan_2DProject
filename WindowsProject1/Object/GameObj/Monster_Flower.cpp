#include "Framework.h"
#include "Monster_Flower.h"

Monster_Flower::Monster_Flower()
	: _sprite(nullptr)
	, _curAction(ActionType::IDLE)
	, _oldAction(ActionType::IDLE)
	, GameObject()
{
	_sprite = new Sprite(L"Resource/Sprites/SpriteFlower.png");
...
void Monster_Flower::Update()
{
	...
	if (_updateTimer <= 0.0f)
	{
		...
		// x축 이동부문
		if (_position.x >= OBJECT->Find(OBJTYPE::PLAYER, 0)->GetPosition().x && !_isAir)
		{
			_sprite->GetTransform()->m_scale.x = 1.0f;
			_moveSpeed = -_speedValue;
		}
		else if (_position.x < OBJECT->Find(OBJTYPE::PLAYER, 0)->GetPosition().x && !_isAir)
		{
			_sprite->GetTransform()->m_scale.x = -1.0f;
			_moveSpeed = _speedValue;
		}
		
		if (_position.y - OBJECT->Find(OBJTYPE::PLAYER, 0)->GetPosition().y <= -5.0f && !_isAir && _movable)
		{
			_jumpSpeed = _maxJumpSpeed;
		}
		_airMoveSpeed = _moveSpeed;
	}
	else
	{
	...
	if (_col->IsCollision(GET_PLAYER->GetAttackCollider()))
	{
		...
		_position.x += GET_PLAYER->GetKnockback();
		_airMoveSpeed = _moveSpeed + GET_PLAYER->GetKnockback() / DELTATIME / 4.0f;
	...
	//x축 이동
	if(_isAir)
		_position.x += _airMoveSpeed * DELTATIME;
	else
...
void Monster_Flower::Render()
{
	_sprite->Render();
	_col->Render();
}

void Monster_Flower::Init()
{
	_hp = _maxHp;
	_sprite->GetTransform()->m_scale.x = 1.0f;
	_moveSpeed = -_speedValue;
...
void Monster_Flower::SpriteInit()
{
	...
	#pragma region 스프라이트 1행
	{
		...
		clips.push_back({ framePos.x += frameWidth,framePos.y,frameWidth, 50.0f, 40.0f * FRAME });
		clips.push_back({ framePos.x += frameWidth,framePos.y,frameWidth, 50.0f, 40.0f * FRAME });
		framePos.x += frameWidth;
...
void Monster_Flower::SetActionType(ActionType type)
{
}

void Monster_Flower::DecreaseHp(int value)
{
	_hp -= value;
	UISTATE->SetTargetHp(_hp, _maxHp);
	if (_hp <= 0)
	{
		EFFECT->Play(L"death_dust", _sprite->GetTransform()->m_pos, Math::Random(0.0f, 360.0f));
		shared_ptr<Gold> temp = dynamic_pointer_cast<Gold>(OBJECT->Generate(PTCTYPE::GOLD, _sprite->GetTransform()->m_pos, 0.0f));
		temp->SetGold(Math::Random(10,20));
		temp->SetMoveSpeed((GET_PLAYER->GetScale().x >= 0 ? 1.0f : -1.0f) * Math::Random(200.0f, 300.0f));
		SetActive(false);
	}
}

void Monster_Flower::ClipEndEvent()
{
}